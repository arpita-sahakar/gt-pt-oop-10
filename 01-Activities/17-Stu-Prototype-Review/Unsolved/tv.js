...
TV.prototype.findActor = function (actor) {
  ...
  axios.get(URL).then((response) => {
    ...
    const { name, birthday, gender, url } = response.data[0].person;
    const showData = `name ${name}, birthday ${birthday}, gender ${gender}, url ${url}`;
    fs.appendFile("log.txt", showData, (err) => {
      err ? console.error(err) : console.log(showData);
    });
  });
};
